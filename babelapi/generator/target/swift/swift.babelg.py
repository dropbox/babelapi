...
#            self._generate_route_class(namespace, route)
    ...
    def _generate_union_type(self, data_type):
        with self.block('enum {}: SerializableAsJSON'.format(self._class_name_for_data_type(data_type))):
            for field in data_type.fields:
    ...
    def _generate_union_serializer(self, data_type):
        with self.block('func serialize() -> String'):
            with self.block('switch self'):
                for field in data_type.fields:
